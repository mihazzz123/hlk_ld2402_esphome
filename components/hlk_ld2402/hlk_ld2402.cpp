...
namespace esphome
{
  namespace hlk_ld2402
  {
    ...
    void HLKLD2402Component::setup()
    {
      ...
      // Настройка UART - явная установка снова
      ...
      // Попытка отправить тестовые данные для проверки функциональности TX
      write_str("TEST\n");
      ESP_LOGI(TAG, "Sent test string to verify TX line");
      delay(500);

      // Очистка любых остаточных данных
      ...
      // ВАЖНОЕ ДОПОЛНЕНИЕ: Убедиться, что устройство запускается в обычном режиме
      // Это предотвращает проблемы с оставшимся инженерным режимом от предыдущих сессий
      ESP_LOGI(TAG, "Setting device to normal mode on startup...");

      // Вход в режим конфигурации - использовать несколько попыток, т.к. устройство может быть в несогласованном состоянии
      ...
      if (config_success)
      {
        // Установка рабочего режима на обычный
        ...
        // Всегда выходить из режима конфигурации
        exit_config_mode_();
        delay(200);
      }
      else
      ...
      // Инициализация, но не трогать устройство, если не нужно
      // Логи показывают, что устройство уже отправляет данные корректно
      // Пропустить конфигурацию и просто настроить датчики
      ...
      // Не проверять помехи питания немедленно - использовать отложенную операцию
      ESP_LOGI(TAG, "Will check power interference and firmware version after 60 seconds");

      // Убрать немедленную проверку версии прошивки - это произойдет через 60 секунд
      // get_firmware_version_();

      // Установить версию по умолчанию - она будет отображаться, пока не определим фактическую версию
      if (firmware_version_text_sensor_ != nullptr)
      {
        firmware_version_text_sensor_->publish_state("HLK-LD2402");
      }

      // Начать пассивное обнаружение версии немедленно
      begin_passive_version_detection_();

      // Установить начальный текст рабочего режима
      operating_mode_ = "Normal";
      publish_operating_mode_();

      // Инициализировать временную метку троттлинга для избежания обновлений сразу после загрузки
      last_distance_update_ = millis();
    }

    // Новая функция для пассивного мониторинга вывода для информации о версии
    void HLKLD2402Component::begin_passive_version_detection_()
    {
      ESP_LOGI(TAG, "Starting passive version detection");
      firmware_version_ = "HLK-LD2402"; // Запасная версия по умолчанию
    }

    // Добавление метода для публикации рабочего режима
    ...
    void HLKLD2402Component::loop()
    {
      ...
      if (millis() - last_debug_time > 30000)
      { // Every 30 seconds
      ...
      while (available())
      {
        ...
        if (c == DATA_FRAME_HEADER[0])
        {
          ...
          if (available() >= 4)
          { // Need at least 4 more bytes to verify the header and frame type
      ...
      {
        uint32_t now = millis();
        if (now - last_calibration_check_ >= 5000)
    ...
    bool HLKLD2402Component::enter_config_mode_()
    {
      ...
      for (int attempt = 0; attempt < 3; attempt++)
      {
        ...
        { // 1 second timeout
          if (available() >= 12)
          { // Minimum expected response size with header/footer
            std::vector<uint8_t> response;
            if (read_response_(response))
            {
